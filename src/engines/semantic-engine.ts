...
export class SemanticEngine {
  ...
  private currentExcludeSignature: string | undefined;
  ...
  private applyExcludes(excludes?: string[]): void {
    const signature = (excludes || [])
      .filter(Boolean)
      .map((s) => s.trim())
      .filter((s) => s.length > 0)
      .join(",");

    if (this.currentExcludeSignature === signature) {
      return;
    }

    this.currentExcludeSignature = signature.length > 0 ? signature : undefined;

    if (this.currentExcludeSignature) {
      process.env.IN_MEMORIA_EXCLUDE = this.currentExcludeSignature;
    } else {
      delete process.env.IN_MEMORIA_EXCLUDE;
      delete process.env.IN_MEMORIA_EXCLUDES;
    }

    // Reset analyzer and caches so new excludes take effect
  ...
  async analyzeCodebase(
    ...
    return globalProfiler.timeAsync(
      ...
      async () => {
        if (options?.excludes) {
          this.applyExcludes(options.excludes);
        }

        // Check cache first
        const cacheKey = `codebase:${path}:${
          this.currentExcludeSignature || ""
        }`;
  ...
  > {
    try {
      ...
      try {
        ...
        if (options?.progressCallback && estimatedFiles > 0) {
      ...
      const timeoutPromise = new Promise<never>((_, reject) => {
        let elapsed = 0;
        const timeoutEnv = process.env.IN_MEMORIA_LEARN_TIMEOUT_MS;
        const timeoutDuration = timeoutEnv ? Number(timeoutEnv) : 1_200_000; // default 20 minutes
        ...
        progressTimer = setInterval(() => {
          ...
          if (elapsed >= timeoutDuration) {
            ...
            reject(
              new Error(
                `Learning process timed out after ${timeoutMinutes} minutes. This can happen with very large Svelte/Vue codebases.`
              )
            );
          } else if (options?.progressCallback && estimatedFiles > 0) {
  ...
  async updateFromAnalysis(analysisData: any): Promise<void> {
    try {
      // Step 1: Ensure the Rust analyzer is ready before propagating updates
      await this.initializeRustAnalyzer()

      if (!this.rustAnalyzer) {
        console.warn('Rust analyzer unavailable; skipping update propagation for analysis data')
        return
      }

      const updatePayload = JSON.stringify(analysisData)

      await this.rustCircuitBreaker.execute(
        async () => {
          await this.rustAnalyzer!.updateFromAnalysis(updatePayload)
        },
        async () => {
          console.warn('Rust analyzer unavailable during update propagation; continuing without updating analyzer')
        }
      )
  ...
  > {
    ...
    const rustImplementation = async () => {
      const frameworkInfo = await FrameworkDetector.detectFrameworks(
        projectPath
      );
