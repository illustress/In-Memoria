...
import {
  assessArchitecturalDecision,
  extractDecisionContext,
  type ChangeAnalysis,
} from "../utils/architectural-decision-detector.js";
...
export class LearningService {
  ...
  static async learnFromCodebase(
    ...
    try {
      ...
      const codebaseAnalysis = await projectSemanticEngine.analyzeCodebase(
        path,
        {
      ...
      insights.push(
        `   ✅ Detected languages: ${
          codebaseAnalysis.languages?.join(", ") || "unknown"
        }`
      );
      insights.push(
        `   ✅ Found frameworks: ${
          codebaseAnalysis.frameworks?.join(", ") || "none detected"
        }`
      );
      if (codebaseAnalysis.complexity) {
        insights.push(
          `   ✅ Complexity: ${
            codebaseAnalysis.complexity.cyclomatic?.toFixed(1) || "N/A"
          } cyclomatic, ${
            codebaseAnalysis.complexity.cognitive?.toFixed(1) || "N/A"
          } cognitive`
        );
      }
      ...
      const concepts = await projectSemanticEngine.learnFromCodebase(path, {
        excludes: options.excludes,
      ...
      if (featureMaps.length > 0) {
        ...
        insights.push(
          `   🔍 Debug: Sample feature map: id=${sample?.id}, featureName=${
            sample?.featureName
          }, primaryFiles=${sample?.primaryFiles?.length || 0}`
        );
      }
      ...
      // Phase 9: Detect and record architectural decisions
      insights.push("🏗️  Phase 9: Detecting architectural decisions...");
      const architecturalDecisionsCount =
        await this.detectAndRecordArchitecturalDecisions(
          path,
          concepts,
          patterns,
          codebaseAnalysis,
      ...
      if (architecturalDecisionsCount > 0) {
        insights.push(
          `   ✅ Detected and recorded ${architecturalDecisionsCount} architectural decision(s)`
        );
      } else {
        insights.push(`   ℹ️  No significant architectural decisions detected`);
      }
  ...
  private static async generateLearningInsights(
    ...
    if (languages.length === 1) {
      ...
      insights.push(
        `🌐 Multi-language codebase (${languages.join(
          ", "
