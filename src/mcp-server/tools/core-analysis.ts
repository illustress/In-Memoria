...
export class CoreAnalysisTools {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            excludes: {
              ...
              items: { type: "string" },
              description:
                "Optional path substrings to exclude during analysis (e.g., dist,node_modules,build)",
            },
          },
          required: ["path"],
        },
  ...
  async analyzeCodebase(args: {
    ...
    try {
      ...
      if (stats.isFile()) {
        ...
        const semanticConcepts = await this.semanticEngine.analyzeFileContent(
          args.path,
          content
        );
              ...
              try {
                ...
                if (
                  ...
                  results.push({
                    ...
                    content:
                      pattern.patternContent.description || pattern.patternType,
  ...
  private async gatherIntelligentAnalysis(path: string) {
    try {
      ...
      const semanticConcepts = await this.semanticEngine.learnFromCodebase(
        path
      );
  ...
  private async generateOverview(analysis: CodebaseAnalysis): Promise<string> {
    const lines = [
      `This documentation provides an intelligent analysis of the **${
        analysis.path || "codebase"
      }**.`,
      "",
    ...
    if (analysis.complexity) {
      lines.push(
        `- **Cyclomatic Complexity**: ${
          analysis.complexity.cyclomatic || "N/A"
        }`
      );
      lines.push(
  ...
  private async generateArchitectureSection(
    ...
    if (analysis.patterns && analysis.patterns.length > 0) {
      const structuralPatterns = analysis.patterns.filter(
        (p) => p.type?.includes("structure") || p.type?.includes("organization")
      );
      ...
      if (structuralPatterns.length > 0) {
        ...
        for (const pattern of structuralPatterns.slice(0, 5)) {
          const patternName =
            pattern.type
              ?.replace(/_/g, " ")
              .replace(/\b\w/g, (l) => l.toUpperCase()) || "Pattern";
          lines.push(
            `- **${patternName}**: ${
              pattern.description || "Structural pattern detected"
            }`
          );
        }
        lines.push("");
      ...
      const designPatterns = analysis.patterns.filter(
        (p) => p.type?.includes("implementation") || p.type?.includes("pattern")
      );
      ...
      if (designPatterns.length > 0) {
        ...
        for (const pattern of designPatterns.slice(0, 5)) {
          ...
          lines.push(
            `- **${patternName}**: ${
              pattern.description || "Design pattern detected"
            }`
          );
        }
        lines.push("");
  ...
  private async generatePatternsSection(
    ...
    if (patternsByCategory.naming) {
      ...
      for (const pattern of patternsByCategory.naming) {
        ...
        lines.push(
          `- **${pattern.type.replace(/naming_/, "").replace(/_/g, " ")}**: ${
            pattern.description
    ...
    if (patternsByCategory.implementation) {
      ...
      for (const pattern of patternsByCategory.implementation) {
        ...
        lines.push(
          `- **${pattern.type
            .replace(/implementation_/, "")
            .replace(/_/g, " ")}**: ${pattern.description}`
        );
      ...
      for (const pattern of structuralPatterns) {
        ...
        lines.push(
          `- **${pattern.type?.replace(/_/g, " ")}**: ${
            pattern.description || "Detected by AI analysis"
      ...
      for (const pattern of implementationPatterns) {
        ...
        lines.push(
          `- **${pattern.type
            ?.replace(/implementation_/, "")
            .replace(/_/g, " ")}**: ${
            pattern.description || "Pattern detected"
          } (${confidence}% confidence)`
        );
      }
      lines.push("");
  ...
  private async generateDiscoveredPatterns(analysis: any): Promise<string> {
    ...
    if (patternsByCategory.naming) {
      ...
      for (const pattern of patternsByCategory.naming) {
        ...
        lines.push(
          `- **${pattern.type?.replace(/naming_/, "").replace(/_/g, " ")}**: ${
            pattern.description || "Naming pattern"
          }`
        );
      ...
      for (const pattern of patternsByCategory.implementation) {
        ...
        lines.push(
          `- **${pattern.type
            ?.replace(/implementation_/, "")
            .replace(/_/g, " ")}**: ${
            pattern.description || "Implementation pattern"
          }`
        );
    ...
    if (analysis.patternInsights.length > 0) {
      ...
      for (const insight of analysis.patternInsights.slice(0, 5)) {
        ...
        lines.push(
          `- **${insight.patternType?.replace(/_/g, " ")}**: Found in ${
            insight.frequency || 1
          } locations (${confidence}% relevance)`
        );
      }
    }
  ...
  private async generateSemanticConcepts(analysis: any): Promise<string> {
    ...
    const conceptsWithRelationships = analysis.semanticConcepts.filter(
      (c: any) => c.relationships && Object.keys(c.relationships).length > 0
    );
    ...
    if (conceptsWithRelationships.length > 0) {
      ...
      for (const concept of conceptsWithRelationships.slice(0, 5)) {
        ...
        lines.push(
          `- **${
            concept.name
          }**: Connected to ${relationshipCount} other concept${
  ...
  private async generateComplexityIntelligence(analysis: any): Promise<string> {
    ...
    if (analysis.codebaseAnalysis.complexity) {
      ...
      if (analysis.semanticConcepts.length > 0) {
        ...
        lines.push(
          `- **Concept density**: ${avgConceptsPerComplexity.toFixed(
            2
          )} concepts per 100 lines`
        );
    ...
    try {
      ...
      if (semanticInsights.insights.length > 0) {
        ...
        for (const insight of semanticInsights.insights) {
          ...
          lines.push(
            `  - **${insight.concept}**: Used in ${contexts} context${
              contexts > 1 ? "s" : ""
            } (confidence: ${insight.usage.frequency}%)`
          );
        }

        // Analyze concept relationships
        const conceptsWithRelationships = semanticInsights.insights.filter(
      ...
      if (patternRecs.recommendations.length > 0) {
        ...
        for (const rec of patternRecs.recommendations.slice(0, 3)) {
          lines.push(
            `  - **${rec.pattern.split("_").slice(1).join(" ")}**: ${
              rec.description
      ...
      const approach = await this.intelligenceTools.predictCodingApproach({
        ...
        context: {
          complexity_cyclomatic:
            analysis.codebaseAnalysis.complexity?.cyclomatic?.toString() || "0",
          complexity_cognitive:
            analysis.codebaseAnalysis.complexity?.cognitive?.toString() || "0",
      ...
      return [
        ...
        `- **Complexity score**: ${
          analysis.codebaseAnalysis.complexity?.cyclomatic || "N/A"
        }`,
        "",
      ...
      if (semanticInsights.insights.length > 0) {
        ...
        for (const insight of semanticInsights.insights) {
          ...
          if (
            ...
            lines.push(
              `const ${insight.concept.toLowerCase()} = new ${
                insight.concept
              }();`
            );
            lines.push(
