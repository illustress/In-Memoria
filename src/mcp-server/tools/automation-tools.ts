...
export class AutomationTools {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            excludes: {
              ...
              items: { type: "string" },
              description:
                "Optional path substrings to exclude during learning (e.g., dist,.webpack,node_modules)",
            },
            skipLearning: {
              type: "boolean",
              description:
  ...
  async autoLearnIfNeeded(args: {
    ...
    try {
      ...
      try {
        ...
        const timeoutPromise = new Promise<never>((_, reject) => {
          timeoutId = setTimeout(() => {
            reject(
              new Error(
                "Semantic analysis timed out after 5 minutes. This often happens with large projects."
              )
            );
          }, 300000); // 5 minutes
        });
        ...
        if (analysisTime > 120000) {
          // More than 2 minutes
          console.error(
            `\n⚠️  Semantic analysis took ${Math.round(
              analysisTime / 1000
            )}s. Consider excluding large generated files.`
          );
        }
      } catch (error) {
        tracker.complete("semantic_analysis");
      ...
      tracker.updateProgress(
        "indexing",
        1,
        "Indexing concepts and patterns..."
      );

      // Indexing happens in-memory, mark progress
      tracker.updateProgress(
        "indexing",
        Math.floor(files.codeFiles / 2),
  ...
  async getLearningStatus(args: { path?: string }): Promise<any> {
    ...
    try {
      ...
      const isStale = hasIntelligence
        ? await this.detectStaleness(projectPath, concepts, patterns)
        : false;
  ...
  private getLastLearningTime(): string | null {
    ...
    try {
      const latestConcept = this.database
        .getSemanticConcepts()
        .sort(
          (a, b) =>
      ...
      const latestPattern = this.database
      ...
      if (latestConcept) {
        latestTime = Math.max(
          latestTime,
          new Date(latestConcept.createdAt).getTime()
        );
      }
      if (latestPattern) {
        latestTime = Math.max(
          latestTime,
          new Date(latestPattern.createdAt).getTime()
        );
      }
  ...
  private async detectStaleness(
    ...
    try {
      ...
      for (const concept of concepts) {
        const conceptTime = concept.createdAt
          ? new Date(concept.createdAt).getTime()
          : 0;
      ...
      for (const pattern of patterns) {
        const patternTime = pattern.createdAt
      ...
      const mostRecentFileTime = await this.getMostRecentFileModificationTime(
        projectPath
      );
   ...
   */
  private async getMostRecentFileModificationTime(
    projectPath: string
  ): Promise<number | null> {
    try {
